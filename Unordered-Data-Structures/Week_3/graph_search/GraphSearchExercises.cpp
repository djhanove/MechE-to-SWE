...
int GridGraph::countEdges() const {
  ...
  for (const auto& keyValue : adjacencyMap) {
    const auto& p1 = keyValue.first;
    const auto& p1_neighbors = keyValue.second;
...
void GridGraph::removePoint(const IntPair& p1) {
  ...
  GridGraph::NeighborSet originalNeighbors = adjacencyMap.at(p1);
    ...
    while (it != originalNeighbors.end()) {
      ...
      while(it2 != adjacencyMap.at(*it).end()) {
        if (it2->first == p1.first && it2->second == p1.second) {
          // erase() function returns the iterator of the next
          // to last deleted element.
          it2 = adjacencyMap.at(*it).erase(it2);
          std::cout<< "erased point 1 from neighbor point" << std::endl;
        } 
        else
          it2++;
      }

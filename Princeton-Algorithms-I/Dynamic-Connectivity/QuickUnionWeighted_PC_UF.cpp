#include "QuickUnionWeighted_PC_UF.h"

quickUnionWeightedPCUF::quickUnionWeightedPCUF(int N)
{
    dim = N;
    for(int i=0; i<N; i++)
    {
...
quickUnionWeightedPCUF::~quickUnionWeightedPCUF(){}

void quickUnionWeightedPCUF::unionPQ(int p, int q)
{
    if(root(p) == root(q))
        return; //Same parent
...
bool quickUnionWeightedPCUF::find(int p, int q)
{
  return root(p) == root(q);  
}

int quickUnionWeightedPCUF::root(int i)
{
    /*

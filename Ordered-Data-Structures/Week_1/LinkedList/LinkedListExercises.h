...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  while (curr)
  {
    ...
    else
    {
      ...
      // update the tail end of the current node
      curr->prev = temp; 

      // update the head end of the current node
      curr = temp->prev;
      curr->next = temp;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  Node* curr_i = left.head_;
  Node* curr_j = right.head_;
  // Use 2 ptr approach and push to back of merged list
  while (curr_i && curr_j)
  {
    if (curr_i->data < curr_j->data)
    {
      merged.pushBack(curr_i->data);
      curr_i = curr_i->next;
    }
    else
    {
      merged.pushBack(curr_j->data);
      curr_j = curr_j->next;
    }    
  }
